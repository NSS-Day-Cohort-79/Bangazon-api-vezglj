...
class OrderTests(APITestCase):
    ...
    def test_add_payment_type_to_order(self):
        ...
        """
        self.test_add_product_to_order()
        #This part will create a payment type
        url = "/paymenttypes"
        data = {
            "merchant_name": "Visa",
        ...
        #This part of the code will PUT payment type onto the open order
        url = "/orders/1"
        ...
        #Time to GET order and verify the payment type is assigned 
        url = "/orders/1"
        self.client.credentials(HTTP_AUTHORIZATION="Token " + self.token) 
        response = self.client.get(url, None, format="json")
        json_response = json.loads(response.content)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertIn(f"/paymenttypes/{payment_type_id}", json_response["payment_type"])
