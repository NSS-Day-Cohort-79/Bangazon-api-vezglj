...
| Ticket # | Title | Why it must go first |
|----------|-------|----------------------|
...
> Once Wave 1 bugs are fixed, write tests to confirm the fixes work and prevent regressions. These tests also cover existing features that currently have no tests.
...
| Ticket # | Title | Why it must go before others |
|----------|-------|------------------------------|
...
> These all require Wave 4 to be done, or depend on earlier Wave 5 items.

| Ticket # | Title | Depends on |
|----------|-------|------------|
| **#17** | Feature: Users can like/dislike products | Wave 1 (products must not crash). Adds like/unlike endpoints + liked products list to profile. |
...
| Ticket # | Title | Depends on |
|----------|-------|------------|
| **#30** | Feature: Filter products by category | Needs #11 (categories must display). Fix the filter dropdown to show real categories instead of placeholder data. |
...
> Lower-priority reporting features and the final store display feature. None are blocking anything else — do these last.
...
Wave 1 Bugs (#3, #4, #5, #6, #8, #9, #10)
  ├──> Wave 2 Learning Spikes (#1, #2)  ← do in parallel with Wave 1
...
Split Wave 1 across the team so all critical bugs are fixed in parallel:
...
| Dev 5 | #2 (TanStack Query spike) — research & present findings to team |